...
const routes = [
    ...
    { 
        from: 'pleiku', 
        to: 'dong_dang', 
        path: [[13.9833, 108.0000], [14.3500, 108.0000], [16.0471, 108.2068], [16.4637, 107.5909], [17.4686, 106.6242], [18.6734, 105.6813], [19.8067, 105.7851], [21.0285, 105.8542], [21.2731, 106.1946], [21.9438, 106.6972]], 
        modes: [{ type: 'truck', icon: '🚚', name: 'Đường bộ', cost: 665.0, time: 2.1, co2: 598.5, distance: 1330, color: 'blue' }] 
    },
    { 
        from: 'dong_dang', 
        to: 'istanbul', 
        path: [[21.9438, 106.6972], [34.3416, 108.9398], [44.1300, 80.4200], [43.2220, 76.8512], [40.4093, 49.8671], [41.0082, 28.9784]], 
        modes: [{ type: 'rail', icon: '🚂', name: 'Đường sắt & Phà (Middle Corridor)', cost: 3200.0, time: 18.0, co2: 1200.0, distance: 4764, color: 'green' }] 
    },
    { 
        from: 'tan_son_nhat', 
        to: 'doha', 
    ...
    { 
        ...
        modes: [{ type: 'ship', icon: '🚢', name: 'Đường biển (Kênh Suez)', cost: 1838.9, time: 26.0, co2: 3677.7, distance: 12259, color: 'blue' }] 
    },
    { 
        from: 'doha', 
        to: 'istanbul', 
...
function initMap() {
    ...
    currentLocationMarker = L.circleMarker([currentHub.lat, currentHub.lng], {
        radius: 9,
        fillColor: '#2980b9',
        color: '#ffffff',
        weight: 3,
...
function updateStatsUI() {
    ...
    if (elCost) elCost.innerText = totalCost.toLocaleString('en-US');
    if (elTime) elTime.innerText = totalTime.toFixed(1);
    if (elCo2) elCo2.innerText = totalCO2.toLocaleString('en-US');
    if (elDist) elDist.innerText = totalDistance.toLocaleString('en-US');
}
