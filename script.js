...
function startGame() {
    ...
    if (mapScreen) mapScreen.style.setProperty('display', 'block', 'important');

    try {
        sessionStorage.setItem('appStarted', 'true');
        if (!sessionStorage.getItem('timerEndTime')) {
...
function startTimer() {
    ...
    let endTime = 0;
    try {
        endTime = parseInt(sessionStorage.getItem('timerEndTime')) || (Date.now() + 10 * 60 * 1000);
    } catch(e) {
